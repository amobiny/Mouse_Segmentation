...
class FCN(BaseModel):
    ...
    def build_network(self):
        ...
        with tf.variable_scope('FCN'):
            conv1 = conv_2d(self.x, self.k_size, 16, 'CONV1', batch_norm=self.conf.use_BN,
                            is_train=self.is_training, activation=self.act_fcn)
            conv2 = conv_2d(conv1, self.k_size, 32, 'CONV2', batch_norm=self.conf.use_BN,
                            is_train=self.is_training, activation=self.act_fcn)
            conv3 = conv_2d(conv2, self.k_size, 64, 'CONV3', batch_norm=self.conf.use_BN,
                            is_train=self.is_training, activation=self.act_fcn)
            ...
            conv7 = conv_2d(conv6, self.k_size, 16, 'CONV7', batch_norm=self.conf.use_BN,
                            is_train=self.is_training, activation=self.act_fcn)
            self.logits = conv_2d(conv7, 1, self.conf.num_cls, 'CONV8', batch_norm=self.conf.use_BN,
