...
from model.ops import conv_2d, deconv_2d, BN_Relu_conv_2d, max_pool, batch_norm, Relu, drop_out, avg_pool, concatenation
...
class DenseNet(BaseModel):
    ...
    def build_network(self, x_input):
        ...
        with tf.variable_scope('DenseNet'):
            ...
            x = conv_2d(x_input, filter_size=3, num_filters=self.trans_out, stride=1, layer_name='conv1',
                        batch_norm=self.conf.use_BN, is_train=self.is_training, add_reg=self.conf.use_reg)
    ...
    def bottleneck_block(self, x, scope):
        with tf.variable_scope(scope):
            ...
            x = Relu(x)
            x = conv_2d(x, filter_size=1, num_filters=4 * self.k, layer_name='conv1', add_reg=self.conf.use_reg)
            x = drop_out(x, keep_prob=self.keep_prob)

            x = batch_norm(x, is_training=self.is_training, scope='BN2')
            x = Relu(x)
            x = conv_2d(x, filter_size=3, num_filters=self.k, layer_name='conv2', add_reg=self.conf.use_reg)
            x = drop_out(x, keep_prob=self.keep_prob)
            return x
    ...
    def transition_down(self, x, scope):
        with tf.variable_scope(scope):
            ...
            x = Relu(x)
            x = conv_2d(x, filter_size=1, num_filters=int(x.get_shape().as_list()[-1]*self.theta_down), layer_name='conv', add_reg=self.conf.use_reg)
            x = drop_out(x, keep_prob=self.keep_prob)
    ...
    def transition_up(self, x, out_shape, scope, num_filters=None):
        with tf.variable_scope(scope):
            x = batch_norm(x, is_training=self.is_training, scope='BN')
            x = Relu(x)
            x = conv_2d(x, filter_size=1, num_filters=int(x.get_shape().as_list()[-1]*self.theta_up), layer_name='conv', add_reg=self.conf.use_reg)
            x = drop_out(x, keep_prob=self.keep_prob)
            x = batch_norm(x, is_training=self.is_training, scope='BN')
            x = Relu(x)
            if not num_filters:
